...
class Gemini():
    def __init__(self):
        ...
        self.prompt = """
            ...
            Regras:
            - "endereco" deve conter o máximo de detalhes disponíveis (rua, número, bairro, cidade, estado) em um único texto.
            ...
            Regras:
            - "ano_conclusao" deve ser preferencialmente apenas o ano, no formato "YYYY" (por exemplo, "2020"). Se não for possível determinar, use null. Muitas vezes é a data de quando foi cursado a
            última série do ensino => Fundamental = 9° / Médio = 3°
    ...
    def analisarDocumento(self, url):
        ext = url.lower().split(".")[-1]
        if ext in ["jpg", "jpeg", "png", "tiff"]:
            return self.leituraImage(url)
        elif ext == "pdf":
            return self.leituraPDF(url)
        elif ext == 'docx':
            return self.docx_to_pdf_from_url_word(url)
        else:
            return {"Erro": True, "Motivo": "Tipo de arquivo não suportado"}

    def leituraImage(self, url):
        try:
    ...
    def docx_to_pdf_from_url_word(self, url, pdf_name='DocumentoTransformado.pdf'):
        ...
        return self.leituraPDF(pdf_path)